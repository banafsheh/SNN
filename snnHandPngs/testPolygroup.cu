...
__global__ void testPolygroup(double* a, double* d, double* dpre, double* delay,
	...
	volatile int i = 0, j = 0, k = 0, tf = 0, t = 0, gr = 1;
	//hard code
	int v1[6];
	int v2[6];
	int v3[6];
	...
	int* psp_times = new int[51];
	for (i = 0; i <= 50; i++) {
		psp_times[i] = 0;
	}
	int group_firings[400][3];
	int group_gr[400][6];
	...
	int startPoint = inputIndex * 226 * ((*T) + (*D) + 301);
	...
	for (i = 1; i <= 5; i++) {
		v1[i] = (((int) ceil(inputNeur[anch[1]] + 4) % 360) / 10) * 5 + i;
		v2[i] = (((int) ceil(inputNeur[anch[2]] + 4) % 360) / 10) * 5 + i;
		v3[i] = (((int) ceil(inputNeur[anch[3]] + 4) % 360) / 10) * 5 + i;
		//if (anch[1] == 1 & anch[2]==2 & anch[3]==6) {printf("%d,%d,%d,%f,%f,%f\n",v1[i],v2[i],v3[i],inputNeur[anch[1]],inputNeur[anch[2]],inputNeur[anch[3]]);}
	}

	for (i = 1; i <= 3; i++) {
		t0[i] = inputTime[anch[i]];
		//if (anch[1] == 1 & anch[2]==2 & anch[3]==6) {printf("%d\n",t0[i]);}
	}
	...
	//if (anch[1] == 1 & anch[2]==2 & anch[3]==6) {printf("%d,%d,%d\n",t1,t2,t3);}
	...
	for (int j1 = startPoint;
			j1 <= startPoint + (226 * ((*T) + (*D) + 301)) - 1; j1++) {
		I[j1] = 0.0;
	}
	...
	for (i = 1; i <= 5; i++) {
		...
		I[startPoint + (v1[i] * ((*T) + (*D) + 301) + (++t1))] = 20;
		I[startPoint + (v2[i] * ((*T) + (*D) + 301) + (++t2))] = 20;
		I[startPoint + (v3[i] * ((*T) + (*D) + 301) + (++t3))] = 20;
		//I[startPoint + (i * ((*T) + (*D) + 301) + t)]);
	...
	for (t = 1; t <= (*T); t++) {
		...
		for (i = 1; i <= (*N); i++) {
			...
			v[i] += 0.5
					* ((0.04 * v[i] + 5) * v[i] + 140 - u[i]
							+ I[startPoint + (i * ((*T) + (*D) + 301) + t)]);//I[i][t]
			v[i] += 0.5
					* ((0.04 * v[i] + 5) * v[i] + 140 - u[i]
							+ I[startPoint + (i * ((*T) + (*D) + 301) + t)]);//I[i][t]
			u[i] += a[i] * (0.2 * v[i] - u[i]);
		...
		for (i = 1; i <= (*N); i++) {
			if (v[i] >= 30) {
				...
				for (k = 1; k <= numSynapse; k++) {
					...
					I[startPoint + (temp1 * ((*T) + (*D) + 301) + temp)] =
							I[startPoint + (temp1 * ((*T) + (*D) + 301) + temp)]
									+ s[i * 23 + k];
					//printf("%f\n",s[i*23+k]);
					if (anch[1] == 2 & i == 66) {
				...
				int temp2;
				for (j = 1; j <= 50; j++) {
					temp2 = (int) ppre[i * 51 + j];
					if (temp2 < 0) {
						printf("%d\n", temp2);
					}
					volatile int c1 = last_fired[temp2];
					volatile int c2 = (int) dpre[i * 51 + j];
					volatile int c3 = c1 + c2;
					if (temp2 != 0) {
				...
				//gr = 1;
				for (j = 1; j <= r - 1; j++) {
					volatile int temp3 = ppre[i * 51 + recent[j]];
