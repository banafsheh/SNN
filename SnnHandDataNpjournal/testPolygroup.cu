...
__global__ void testPolygroup(double* a, double* d, double* dpre[50], double* delay[22],
	...
	int inputIndex = blockId;
	anch[0] = anchors[inputIndex * 3][0];
		...
		for (i = 0; i < (*N); i++) {
			if (v[i] >= 30) {
				...
				for (k = 0; k < numSynapse; k++) {
					int temp = pp[i][k] + t * (*N);
					temp = (temp - post[i][k]) / (*N) + 1;
					I[post[i][k]][temp] = I[post[i][k]][temp] + s[i][k];
				}

				//50 is the second dimension of dpre and ppre (hard code)
				//PSP_times= last_fired(ppre{fired(k)}) + dpre{fired(k)}';
				for (j = 0; j < 50; j++) {
					psp_times[j] = last_fired[ppre[i][j]] + dpre[i][j];
				}
				...
				for (j = 0; j < r - 1; j++) {
					group_gr[gr][1] = last_fired[ppre[i][recent[j]]];
					group_gr[gr][2] = ppre[i][recent[j]];
					group_gr[gr][3] = last_fired[ppre[i][recent[j]]]
							+ dpre[i][recent[j]];
					group_gr[gr][4] = i;
					group_gr[gr++][5] = t;
				}
